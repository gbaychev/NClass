...
namespace NClass.Core
{
    public sealed class CommentRelationship : Relationship
    {
        ...
        {
            this.comment = comment ?? throw new ArgumentNullException("comment");
            this.entity = entity ?? throw new ArgumentNullException("entity");
            Attach();
        }

        public override RelationshipType RelationshipType => RelationshipType.Comment;
        ...
        {
            return $"{Strings.Comment}: {comment.ToString()} --- {entity.Name}";
        }
    }
}
